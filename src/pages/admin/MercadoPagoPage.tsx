...
import { Save, Loader2, Check, Eye, EyeOff, Copy, FlaskConical, AlertCircle, CheckCircle2, ShieldCheck } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-neutral-500 mt-1">
          Configure as credenciais do Checkout Transparente em ambiente de producao.
        </p>
      </div>

      <div className="rounded-xl border border-emerald-900/40 bg-emerald-950/20 p-4 flex items-start gap-3">
        <ShieldCheck size={16} className="text-emerald-400 shrink-0 mt-0.5" />
        <div className="text-xs text-emerald-200/90 leading-relaxed">
          <strong className="text-emerald-300">Ambiente de PRODUCAO.</strong> Todas as cobrancas
          serao reais. Use as credenciais APP_USR-... da sua aplicacao no Mercado Pago.
        </div>
      </div>

      {/* Production Credentials */}
      <div className="border border-[#1a1a1a] rounded-xl bg-[#0a0a0a] p-5 space-y-4">
        <div className="text-xs uppercase tracking-wider text-neutral-500">Credenciais de producao</div>

        <div className="rounded-lg bg-[#050505] border border-[#1a1a1a] p-3 text-[11px] text-neutral-400 leading-relaxed">
          As credenciais comecam com <code className="font-mono text-neutral-200">APP_USR-</code>.
          Encontre em "Suas integracoes" {'>'} sua aplicacao {'>'} "Credenciais de producao" no painel
          do Mercado Pago.
        </div>

        <div>
          <label className="text-xs text-neutral-500 mb-2 block">Public Key</label>
          <input
            value={pubKeyProd}
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          onClick={test}
          disabled={testing || !cfg?.has_access_token_prod}
